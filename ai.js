...
let minimax = function(maximizingPlayer, depth, gameArray){
    ...
    if(depth == 0){
        ...
        if(maximizingPlayer){
            ...
            for(let i = 0; i < gamePoss.length; i++){
                ...
                let mini = minimax(!maximizingPlayer,depth-1,gamePoss[i]).value;
                if(mini > val){
                    choice = gamePoss[i];
                }
                val = Math.max(val, mini);
            }
        }else{
